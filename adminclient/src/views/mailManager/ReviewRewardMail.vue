  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <span slot="action" slot-scope="text, record">
        <a @click="openshenhe(record)" v-if="!record.reviewer && !record.reviewTime">审核</a>
        <span v-if="record.reviewer || record.reviewTime">已审核</span>
      </span>
    </s-table>
    ...
    <a-modal title="确认完成审核本邮件" :visible="shenhevisible" @ok="handleshenhe" @cancel="shenhevisible = false">
      <p>{{shenheText}}</p>
    </a-modal>
  </a-card>
</template>
...
import CreateForm from '../system/user/modules/CreateUserForm'
import DetailModal from '../system/user/modules/UserDetailModal'
import ChangeModal from '../system/user/modules/ChangeUserInfo'
import ResetPwdModal from '../system/user/modules/ResetPwdModal'
import {getAccessRight} from '@/api/manage'
import { getListRewardMailReview, reviewRewardMail } from '@/api/user'
...
export default {
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        return getListRewardMailReview(parameter).then(res => {
  ...
  methods: {
    ...
    handleshenhe() {
      let s = this
      reviewRewardMail({ rewardMailId: this.shenheId }).then(res => {
        // console.log('删除好友=======》',res)
        if(res != null) {
          s.$message.success('成功审核奖励邮件')
        }
        s.shenhevisible = false
        s.$refs.table.refresh()
      })
    },
    openshenhe(record) {
      this.shenheText = '确认完成审核邮件:' + record.id
