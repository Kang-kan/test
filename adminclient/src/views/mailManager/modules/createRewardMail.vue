    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="邮件标题" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['title', {rules: [{required: true, min: 2, max: 128, message: '长度为0-128个字符！'}]}]"
          />
        </a-form-item>
        <a-form-item label="邮件内容" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-textarea
            :row="4"
            v-decorator="['content', {rules: [{required: true, min: 2, max: 128, message: '长度为0-1024个字符！'}]}]"
          />
        </a-form-item>
        <a-form-item label="备注" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input
            v-decorator="['remark', {rules: [{required: true, min: 0, max: 32, message: '长度为0-32个字符！'}]}]"
          />
        </a-form-item>
...
import { createRewardMail } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    disabledDate(current) {
      ...
      return current && current < this.$moment().endOf('day') - 1
    },
    disabledDateTime() {
      return {
    ...
    mailTypeChange(value) {
      ...
      console.log(this.playervisible)
    },
    handleSubmit() {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          console.log('创建邮件: ', values)
          values.buildTime = null
          createRewardMail(values).then(result => {
            if (result != null) {
              this.visible = false
              this.confirmLoading = false
              this.$emit('ok', values)
              s.$message.success('成功创建奖励邮件')
            } else {
              this.confirmLoading = false
