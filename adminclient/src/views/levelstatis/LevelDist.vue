...
import { getLevelDistStatis, getServerInfo } from '@/api/user'
export default {
  name: 'LevelDist',
  components: {
    STable,
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        ...
        return getLevelDistStatis(Object.assign(parameter, s.queryParam)).then(res => {
          ...
          res.contents.forEach(el => {
            ...
            let newobj = {}
            el.levelCounts.sort((a, b) => {
              return (a.level % 1000) - (b.level % 1000)
            })
            let total = el.total
            el.levelCounts.forEach(e => {
              e.level > 999
                ? (e.level = Math.floor(e.level / 1000) + '转' + (e.level % 1000) + '级')
                : (e.level = parseInt(e.level) + '级')
              e['countp'] = ((e.count / total) * 100).toFixed(2) + '%'
            })
