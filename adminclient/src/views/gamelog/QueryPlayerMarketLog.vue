    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row>
          ...
          <a-col :md="5" :sm="12">
            <a-row>
              <a-col :md="7" style="line-height: 32px;">道具:</a-col>
              <a-col :md="17">
                <a-select 
                    showSearch
...
import { formatTimestamp, CurrencyTypeName } from '@/utils/util'
import moment from 'moment'
import { STable, Ellipsis, Modal } from '@/components'
import StepByStepModal from '../list/modules/StepByStepModal'
import CreateForm from '../list/modules/CreateForm'
import { getQueryMarketLogById, getQueryMarketLog, getServerInfo } from '@/api/user'
...
export default {
  ...
  data() {
    return {
      description: '查询商城日志，不输入具体玩家信息则必须选择道具，指定玩家后不选择道具则默认查询所有道具',
      selectId: '0',
      itemValue: '',
      showpagi: false,
      currentPage: 0,
      ...
      loadData: parameter => {
        ...
        if (!s.queryParam.startTime || !s.queryParam.endTime || !s.queryParam.serverId) {
        ...
        if (!s.queryParam.accountId && !s.queryParam.playerId && !s.queryParam.playerName) {
          return getQueryMarketLogById(Object.assign(parameter, s.queryParam)).then(res => {
        ...
        return getQueryMarketLog(Object.assign(parameter, s.queryParam)).then(res => {
          //console.log('tablereturn=======>', res)
          if (!res || !res.contents || res.contents.length == 0) {
            return {
              pageSize: 10,
  ...
  methods: {
    ...
    handleSelectChange(value) {
      this.queryParam.itemId = value.split(':')[0]
    },
    serverChange(value) {
