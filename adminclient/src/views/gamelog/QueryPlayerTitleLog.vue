...
<script>
  ...
  import { getPlayerTitleLog, getServerInfo } from '@/api/user'
  import { async } from 'q'
  export default {
    ...
    data() {
      return {
        ...
        loadData: parameter => {
          ...
          return getPlayerTitleLog(Object.assign(parameter, s.queryParam)).then(res => {
            ...
            res.contents.forEach(async function(el) {
              el.expireTime = formatTimestamp(el.expireTime + el.time)
              switch (el.opValue) {
                case 0:
                  el.opValue = '获得'
                  break
              ...
              el.titleId = (await xmlObj.getDesignationName(el.titleId)) + '(' + el.titleId + ')'
            })
            return res
          })
        },
