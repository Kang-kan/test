...
import { getQueryFashionLog, getServerInfo } from '@/api/user'
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          customRender: createTime => (createTime == 0 ? '永久' : formatTimestamp(createTime))
        },
        // {
        //   title: '平台',
        //   dataIndex: 'platName',
      ...
      loadData: parameter => {
        ...
        return getQueryFashionLog(Object.assign(parameter, s.queryParam)).then(res => {
          ...
          res.contents.forEach(el => {
            el.fashionId = s.fashionObj[el.fashionId]
            switch (el.opValue) {
              case 0:
                el.opValue = '激活'
                break
  ...
  mounted: function() {
    ...
    s.$ajax.get('../fashionConfig.xml').then(res => {
      let obj = s.$x2js.xml2js(res.data).def.unit
      let newObj = {}
      obj.forEach(el => {
        obj[el._id] = el._name
      })
