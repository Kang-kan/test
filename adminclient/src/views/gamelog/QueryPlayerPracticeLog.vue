...
<script>
  ...
  import { getPracticeLog, getServerInfo } from '@/api/user'
  export default {
    ...
    data() {
      return {
        ...
        loadData: parameter => {
          ...
          return getPracticeLog(Object.assign(parameter, s.queryParam)).then(res => {
            ...
            res.contents.forEach(el => {
              switch (el.childType) {
                case 0:
                  el.childType = 'CostItemLevelUp'
                  break
                case 1:
                  el.childType = 'CostItemAddExp'
                  break
                case 2:
                  el.childType = 'CostItemLearnSkill'
                  break
                case 3:
                  el.childType = 'GridConfig'
                  break
              }
            })
            res.contents.forEach(async function(el) {
              ...
              let configObj = await xmlObj.getHuoBanName(
                el.opValue,
                el.configId + '',
                el.levelId + '',
                el.childConfigId + ''
              )
              el.opValue = configObj.opName
              el.configId = configObj.configName
              el.levelId = configObj.levelName
              el.childConfigId = configObj.childConfigName
            })
            return res
          })
        },
    ...
    mounted: function() {
      ...
      console.log('服务器地址', s.serverList)
    }
  }
</script>
