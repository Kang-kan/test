  ...
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="0" style="line-height: 50px;">
          <a-col :md="6" :sm="12">
            <a-row>
              <a-col :md="6" style="line-height: 50px;">渠道:</a-col>
              <a-col :md="14">
          ...
          <a-col :md="5" :sm="12">
            <a-row>
              <a-col :md="7" style="line-height: 50px;">区服:</a-col>
              <a-col :md="14">
                <a-select size="default" @change="serverChange" placeholder="默认全渠道">
                  <a-select-option
    ...
    >
      ...
      <span slot="description" slot-scope="text">
        <ellipsis :length="1000" tooltip>{{ text }}</ellipsis>
      </span>
...
<script>
  ...
  import { getQueryPowerLog, getServerInfo } from '@/api/user'
  ...
  export default {
    ...
    data() {
      return {
        description: "只记录战斗力排行榜前20名的玩家，登录、登出时的战斗和属性。",
        showpagi: false,
        currentPage: 0,
        time: null,
        channelIdList: [],
        ...
        columns: [
          ...
          },
          {
            ...
            customRender: action => (action == 1 ? '初始化' : '登出')
          },
          {
            title: '英雄配置id',
            dataIndex: 'configId',
        ...
        loadData: parameter => {
          ...
          // if (!s.queryParam.account && !s.queryParam.playerName && !s.queryParam.playerId) {
          ...
          //   })
          // }
          return getQueryPowerLog(Object.assign(parameter, s.queryParam)).then(res => {
            ...
            //     el.chopId = await xmlObj.getChopName(el.chopId)
            //     el.childId = '碎片' + el.childId
            //   })
            return res
          })
        },
