...
import { getQueryOfflineLog, getServerInfo } from '@/api/user'
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'gap',
          customRender: gap => {
            let day = Math.floor(gap / 86400)
            let hour = Math.floor(gap / 3600) % 24
            let m = Math.floor(gap / 60) % 60
            let s = gap % 60
            let str = ''
            if (day > 0) {
              str = day + '天' + hour + '小时' + m + '分' + s + '秒'
            } else if (day <= 0 && hour > 0) {
              str =hour + '小时' + m + '分' + s + '秒'
            } else if (day <= 0 && hour <= 0 && m > 0) {
              str =m + '分' + s + '秒'
            } else if (day <= 0 && hour <= 0 && m <= 0) {
              str = s + '秒'
            }
      ...
      loadData: parameter => {
        ...
        return getQueryOfflineLog(Object.assign(parameter, s.queryParam)).then(res => {
