...
import { getChargeTotalDist, getServerInfo } from '@/api/user'
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        ...
        return getChargeTotalDist(this.queryParam).then(res => {
          ...
          obj.contents = res
          res.forEach(el => {
            el['amountp'] = (((el.amount * el.playerCount) / el.totalAmount) * 100).toFixed(3) + '%'
            el['playerCountp'] = ((el.playerCount / el.totalPlayer) * 100).toFixed(3) + '%'
          })
          return obj
        })
      },
