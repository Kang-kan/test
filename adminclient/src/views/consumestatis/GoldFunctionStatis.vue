...
import { getGoldFunctionStatis, getServerInfo } from '@/api/user'
import { getRoleList, getServiceList } from '@/api/manage'
export default {
  name: 'GoldFunctionStatis',
  components: {
    STable,
    Ellipsis,
    CreateForm,
    ...
    return {
      ...
      columns: [
        {
          ...
          customRender: dateTime => {
            let year = Math.floor(dateTime / 10000)
            let mon = Math.floor(dateTime / 100) % 100
        ...
        },
        {
          ...
          customRender: gold => gold > 0 ? gold : gold * -1
        },
        {
          title: '花费人数占比',
          scopedSlots: {
      ...
      loadData: parameter => {
        ...
        if(parameter.pageSize == -1) {
          parameter.pageSize = s.totalCount ? s.totalCount : 99999;
          parameter.pageNo = 0;
        }
        ...
        return getGoldFunctionStatis(Object.assign(parameter, s.queryParam)).then(res => {
          ...
          res.contents.forEach(el => {
            ...
            el['functionCountp'] = Math.floor((el.functionCount / el.total) * 100) + '%'
          })
          s.tableData = JSON.parse(JSON.stringify(res.contents));
          s.totalCount = res.totalCount;
          return res
        })
      },
      selectedRowKeys: [],
  ...
  methods: {
    ...
    output() {
      ...
      if(s.tableData && s.tableData.length > 0) {
        let str = "统计日期  服务器id  总人数  行为  花费人数  消耗仙晶  花费人数占比\n";
        s.tableData.forEach(el => {
          let dateTime = el.dateTime
          let year = Math.floor(dateTime / 10000)
          let mon = Math.floor(dateTime / 100) % 100
          let day = dateTime % 100
          ...
          let serverName = ''
          let serverId = el.serverId
          s.serverList.forEach(el => {
              if (el.id == serverId) {
                serverName = el.name + '(' + el.id + ')'
              }
            })
          str += (dateStr + '  ' + serverName + '  ' + el.total + '  ' + el.functionType + '  ' + el.functionCount + '  ' + el.gold + '  ' + el.functionCountp + '\n')
        })
        s.download(`仙晶消耗_page${s.currentPage}.txt`,str)
      }else {
