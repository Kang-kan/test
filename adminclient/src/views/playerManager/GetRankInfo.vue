    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="0">
          <a-col :md="5" :sm="12">
            <a-row>
              <a-col :md="7" style="line-height: 32px;">服务器:</a-col>
              <a-col :md="14">
          ...
          <a-col :md="6" :sm="12">
            <a-row>
              <a-col :md="7" style="line-height: 32px;">排行榜类型:</a-col>
              <a-col :md="14">
                <a-select size="default" @change="typeChange">
                  <a-select-option
                    v-for="i in rankTypeList"
                    :key="i.id"
                    :value="i.id"
                  >{{i.type}}({{i.id}})</a-select-option>
                </a-select>
              </a-col>
            </a-row>
          </a-col>
...
import { getGetRankInfo, getServerInfo } from '@/api/user'
export default {
  name: 'GetRankInfo',
  components: {
    STable,
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        ...
        if (!s.queryParam.rankType && !s.queryParam.serverId) {
        ...
        return getGetRankInfo(Object.assign(parameter, s.queryParam)).then(res => {
          if (!res || res.length == 0) {
            return {
              pageSize: 10,
              pageNo: 0,
  ...
  methods: {
    ...
    typeChange(value) {
      this.queryParam.rankType = Number(value)
    },
    serverChange(value) {
