<template>
  <a-card :bordered="false">
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="$refs.createModal.add()">封号</a-button>
    </div>
    ...
    <s-table ref="table" size="default" :columns="columns" :data="loadData" rowKey="id">
      ...
      <span slot="action" slot-scope="text, record">
        <a href="javascript:;" @click="openDelet(record)">解除封号</a>
      </span>
    </s-table>
...
<script>
  ...
  import CreateForm from './modules/BlockPlayer'
  import { getAccessRight } from '@/api/manage'
  import { getServerInfo, unBlockPlayer, queryBlockPlayers, queryBlockPlayer, queryBlockPlayersBySrv } from '@/api/user'
  ...
  export default {
      ...
      return {
        ...
        loadData: parameter => {
          ...
          if (!s.queryParam.serverId && !s.queryParam.playerId && !s.queryParam.playerName && !s.queryParam.accountId) {
            return queryBlockPlayers(Object.assign(parameter, s.queryParam)).then(res => {
              if (!res || !res.contents) {
                return {
                  pageSize: 10,
                  pageNo: 0,
            ...
            return queryBlockPlayersBySrv(Object.assign(parameter, s.queryParam)).then(res => {
              if (!res || !res.contents) {
                return {
                  pageSize: 10,
                  pageNo: 0,
            ...
            return queryBlockPlayer(Object.assign(parameter, s.queryParam)).then(res => {
              if (!res) {
                return {
                  pageSize: 10,
                  pageNo: 0,
    ...
    methods: {
      ...
      openDelet(record) {debugger
        console.log("玩家信息",record)
        this.deletText = `确认解除玩家:${record.playerName}禁言`
        // this.deletId = record.playerId
