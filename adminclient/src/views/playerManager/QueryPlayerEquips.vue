    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="0">
          ...
          <a-col :md="4" :sm="24" style="padding-right: 18px;">
            <a-input placeholder="请输入武器id" v-model="queryParam.equipId" />
          </a-col>
          <a-col :md="2" :sm="24">
...
import { getQueryPlayerEquips, getServerInfo } from '@/api/user'
import { async } from 'q'
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'wear',
          customRender: wear => (wear == 0 ? '未穿上' : '已穿上')
        }
      ],
      ...
      loadData: parameter => {
        ...
        return getQueryPlayerEquips(Object.assign(parameter, s.queryParam)).then(res => {
          ...
          var newarr = []
          res.forEach(async function(el,index) {
            el.equipId = await xmlObj.getName(el.equipId) + '(' + el.equipId + ')'
            if(!(!isNaN(el.equipId.split('(')[0]) && typeof Number(el.equipId.split('(')[0]) == 'number')){　　　　
        　　　 newarr.push(el)
        　　}
          })
          let obj = {}
          ...
          obj.totalPage = Math.ceil(newarr.length / pageSize)
          obj.totalCount = newarr.length
          obj.contents = newarr
          return obj
        })
      },
