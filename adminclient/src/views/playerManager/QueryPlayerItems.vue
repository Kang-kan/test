    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="0">
          ...
          <a-col :md="4" :sm="24" style="padding-right: 18px;">
            <a-input placeholder="请输入道具id" v-model="queryParam.itemId" />
          </a-col>
          <a-col :md="2" :sm="24">
...
import { getQueryPlayerItems, getServerInfo } from '@/api/user'
import { async } from 'q'
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          ...
          customRender: endTime => (endTime > 0 ? formatTimestamp(endTime) : '无')
        },
        {
          title: '道具的数量',
          scopedSlots: {
      ...
      loadData: parameter => {
        ...
        return getQueryPlayerItems(Object.assign(parameter, s.queryParam)).then(res => {
          ...
          res.forEach(async function(el) {
            el.itemId = await xmlObj.getName(el.itemId) +'(' +el.itemId+')'
          })
          let obj = {}
