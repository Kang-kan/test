...
import { getPlayerList, getServerInfo } from '@/api/user'
export default {
  name: 'PlayerList',
  components: {
    STable,
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'banPlayerTime',
          customRender: banPlayerTime => (banPlayerTime == -1 ? '永久' : (banPlayerTime == 0 ? '无' : formatTimestamp(banPlayerTime)))
        },
        {
          title: '禁言到期时间',
          scopedSlots: {
          ...
          dataIndex: 'banWordTime',
          customRender: banWordTime => (banWordTime == -1 ? '永久' : (banWordTime == 0 ? '无' : formatTimestamp(banWordTime)))
        },
        {
          title: '等级',
          dataIndex: 'level',
        ...
        },
        {
          ...
          customRender: registTime => formatTimestamp(registTime)
        },
        {
          title: '最后登录时间',
          ...
          customRender: lastLoginTime => formatTimestamp(lastLoginTime)
        },
        {
          title: 'ip',
          dataIndex: 'ip',
      ...
      loadData: parameter => {
        ...
        return getPlayerList(Object.assign(parameter, s.queryParam)).then(res => {
