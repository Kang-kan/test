      ...
      <a-form layout="inline">
        <a-row :gutter="0">
          <a-col :md="6" :sm="24">
            ...
            >
              <a-range-picker format="YYYY-MM" @change="handleTimeChange" v-model="time"/>
            </a-form-item>
          </a-col>
          <a-col :md="6" :sm="12">
            <a-row>
...
import { getMonthActiveSummary, getServerInfo } from '@/api/user'
export default {
  name: 'MonthlyActive',
  components: {
    STable,
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        ...
        return getMonthActiveSummary(Object.assign(parameter, s.queryParam)).then(res => {
  ...
  mounted: function() {
    ...
    this.$http.get('/roleGameServer/getMyServers').then(res => {
      ...
      if (!res) {
        ...
        let endTime = s.$moment().format('YYYYMM')
        s.time = [s.$moment(startTime.toString(), 'YYYYMM'), s.$moment(endTime, 'YYYYMM')]
        s.timeArr[0] = startTime
        s.timeArr[1] = endTime
        return
      }
      let startTime = s.$moment(res.openTime).format('YYYYMM')
      let endTime = s.$moment().format('YYYYMM')
      console.log('endTime: ', endTime)
      s.time = [s.$moment(startTime, 'YYYYMM'), s.$moment(endTime, 'YYYYMM')]
      s.timeArr[0] = startTime
      s.timeArr[1] = endTime
    })
    })
