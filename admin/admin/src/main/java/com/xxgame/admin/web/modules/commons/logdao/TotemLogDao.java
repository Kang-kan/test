 ...
 */
@Service
public class TotemLogDao extends AbstractDao {
    ...
    public PageDto<Map<String, Object>> findPlayerLog(long playerId, int pageNo, int pageSize) {
        String sql = "SELECT * FROM totem_log WHERE player_id = ? ORDER BY id DESC";
        String countSql = "SELECT COUNT(*) FROM totem_log WHERE player_id = ?";
