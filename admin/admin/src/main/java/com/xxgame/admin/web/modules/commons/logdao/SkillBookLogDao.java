 ...
 */
@Service
public class SkillBookLogDao extends AbstractDao {
    ...
    public PageDto<Map<String, Object>> findPlayerLog(long playerId, int pageNo, int pageSize) {
        String sql = "SELECT * FROM skill_book_log WHERE player_id = ? ORDER BY id DESC";
        String countSql = "SELECT COUNT(*) FROM skill_book_log WHERE player_id = ?";
